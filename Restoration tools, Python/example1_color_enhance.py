...
#-------------------------------------------------------------------------------------------------------
def example1_color_enhance (img=None):
    if (img==None):  img = cv2.imread ('original.tif') 
    if (img==None):  print 'Image file data not found and read; Abort'; return None, None
    rip = retinal_image_processing()
    rip.set_image(img)
    imgLAB = cv2.cvtColor (rip.img, cv2.COLOR_BGR2LAB)
    rip.set_image_channel(imgLAB[:,:,0].copy())
    hist, histcum, nbins = rip.extract_channel_histogram ()
    histcum = histcum / histcum.max()
    mn = histcum[histcum<0.01].size
    mx = histcum[histcum<0.99].size
    tcurve = hist.copy()    # Create linear tone curve between intensities 1% to 99%
    tcurve[0:mn-1] = 0
    for i in range(mn,mx):       tcurve[i] = (nbins-1)*(i-mn)/(mx-mn)
    tcurve[mx:nbins-1] = (nbins-1)
    imgChan = rip.apply_channel_tone_curve (tcurve)
    imgLAB[:,:,0] = imgChan[:,:,0]
    imgEnhance = cv2.cvtColor (imgLAB, cv2.COLOR_LAB2BGR)
    return img, imgEnhance

def example1_color_enhance_explicit (img=None):
    if (img==None):  img = cv2.imread ('original.tif') 
    if (img==None):  print 'Image file data not found and read; Abort'; return None, None
    imgLAB = cv2.cvtColor (img, cv2.COLOR_BGR2LAB)
    imgChannel = imgLAB[:,:,0].copy()
    nbins = 100
    if imgChannel.dtype == np.uint8:  nbins = 256
    if imgChannel.dtype == np.uint16: nbins = pow(2,16)
    hist = cv2.calcHist ([imgChannel], [0], None, [nbins], [0,nbins])
    histcum = hist.cumsum()   
    histcum = histcum / histcum.max()
    mn = histcum[histcum<0.01].size
    mx = histcum[histcum<0.99].size
    ...
    for i in range(mn,mx):       tcurve[i] = (nbins-1)*(i-mn)/(mx-mn)
    tcurve[mx:nbins-1] = (nbins-1)
    tmp = tcurve [imgChannel]
    imgChannel = tmp.astype(imgChannel.dtype)    
    imgLAB[:,:,0] = imgChannel[:,:,0]
    imgEnhance = cv2.cvtColor (imgLAB, cv2.COLOR_LAB2BGR)
    return img, imgEnhance
...
if __name__ == "__main__":
    ...
    res1, res2 = example1_color_enhance()
    res3, res4 = example1_color_enhance_explicit()
    im1 = np.hstack((res1, res2))
    iv1 = np.hstack((res3, res4))
    im1 = np.vstack((im1, iv1))
    display ('Example 1 color / enhance image', im1)
    cv2.imwrite('example1.png', im1)
    cv2.imwrite('color_output.png', res2)
